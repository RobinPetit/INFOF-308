...
\section{Results}
	...
	\subsection{Examples}
		...
		\begin{lemma}[Upper boundary of edges amount for $k=2$] For $m > \frac {\abs V}2$, we have $\Lambda_2^m(V, \cdot) = \emptyset$.
		\end{lemma}
		...
		$\Gamma(V, E)$ such that $\abs E = m$, and take $\mathcal V_1 \coloneqq \{v \in V \st \deg(v) \leq 1\} \subset V$. The restriction
		...
		Thus $\{v, v_{i_1}, v_{i_k}\}$ is a connected component of size $k+1$. Thus the largest connected component must have size at least that big.
		\end{proof}
