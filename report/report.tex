...
component of a subgraph in order to avoid simulation time for the computation of the z-score of the clustering analysis.
...
\section{Results}
	...
	interactome.
		...
		separation score (Figure~\ref{fig:s_AB histogram}). We expected the disease pairs with non common gene to also be separated in the network ($s_{AB} > 0$) and disease pairs with one being a complete subset of the other to be overlapping ($s_{AB} < 0$). We found similar results to the ones in the
		...
		great diversity of separating scores. In order to understand why we observed these pairs with separation scores not meeting our expectations, it would be interesting to further investigate them.
	...
	new dataset.
		...
		estimated interactions of the interactome \citep{estimatingTheSizeOfTheHumanInteractome,ATruerMeasureOfOurIgnorance}. By defining the density of a graph $\Gamma = (V, E)$ as being $d(\Gamma) \coloneqq \abs E/\binom {\abs V}2$, the new interactome is more than 1.5 times denser than the original one with $0.156\%$ versus $0.234\%$
		for the new one.
...
\section{Analytical subgraph LCC distribution}
...
\section{Materials and Methods}
	...
	\subsection{Databases}
		...
		%The outputted file by Inter-build and the interactome provided with the original paper both use Entrez gene IDs, they can therefore easily be merged. In order to merge these, the script \texttt{merger.py} has been written. As these two tsv files have a different format (different columns), only the gene IDs are outputted by \texttt{merger.py}.
		...
		%The other databases of the original interactome were not included in the new interactome because they were not available for download in a Inter-build-compatible format.
